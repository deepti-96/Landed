...
export default function RoadmapSection({ title, subtitle, steps, profile, onStepClick }: Props) {
  ...
  return (
    <section className="relative rounded-[32px] border border-slate-200 bg-white/90 p-6 shadow-sm shadow-slate-200/40 backdrop-blur">
      <div className="mb-6 flex flex-col gap-3 border-b border-slate-100 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Milestone</p>
          <h2 className="mt-2 text-2xl font-semibold text-slate-900">{title}</h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500">{subtitle}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-500">
          <span className="font-semibold text-slate-900">{completed}</span> of <span className="font-semibold text-slate-900">{steps.length}</span> completed
        </div>
      </div>

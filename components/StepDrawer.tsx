        ...
        <div className="flex items-start justify-between p-6 border-b border-gray-100">
          <div className="flex-1 pr-4">
            <div className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium mb-2 ${
              step.status === 'available' ? 'bg-blue-50 text-blue-600' :
        ...
        <div ref={explanationRef} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {step.forms && step.forms.length > 0 && (
            <div className="bg-gray-50 rounded-2xl p-4">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Forms needed</p>
              <div className="flex flex-wrap gap-2">
                {step.forms.map(form => (
                  <span key={form} className="text-sm font-mono bg-white border border-gray-200 text-gray-700 px-3 py-1 rounded-lg">
                    {form}
                  </span>
                ))}
              </div>
          ...
          {!showDraft ? (
            ...
            <div className="border border-gray-200 rounded-2xl overflow-hidden">
              <div className="p-4 border-b border-gray-100 bg-gray-50">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Draft type</p>
                <div className="flex flex-wrap gap-2">
                  {DRAFT_TYPES.map(dt => (
                    <button
                      key={dt.value}
                ...
                <button
                  ...
                  className="mt-3 w-full bg-gray-900 text-white rounded-xl py-2 text-sm font-medium hover:bg-gray-800 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                >
                  {loadingDraft ? (
              ...
              {draft && (
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Your draft</p>
                    <button
                      onClick={copyDraft}
